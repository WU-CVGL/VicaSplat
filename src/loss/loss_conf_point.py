...
class Regr3D(nn.Module):
    ...
    def forward(
        ...
        pixel_loss_1 = torch.norm(gt_pts1 - pr_pts1, dim=-1)
        pixel_loss_2 = torch.norm(gt_pts2 - pr_pts2, dim=-1)
        conf_pixel_loss_1 = gt_conf1 * pixel_loss_1
        conf_pixel_loss_2 = gt_conf2 * pixel_loss_2
        conf_pixel_loss = conf_pixel_loss_1[valid1].mean() + conf_pixel_loss_2[valid2].mean()

        if pr_conf1 is not None and pr_conf2 is not None:
            conf_loss = torch.abs(pr_conf1 - gt_conf1).mean() + torch.abs(pr_conf2 - gt_conf2).mean()
            return conf_pixel_loss + conf_loss
        
        return conf_pixel_loss
