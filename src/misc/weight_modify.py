...
def checkpoint_filter_fn(
    ...
    for k, v in state_dict.items():
        if f'backbone.{k}' not in model_state_dict and "downstream_head1" not in k:
            continue
    ...
    drop_conf = out_dict['downstream_head1.dpt.head.4.bias'].shape[0] == 4 and model_state_dict['downstream_head1.dpt.head.4.bias'].shape[0] == 3
    if drop_conf:
...
#     for k, v in state_dict.items():
#         if 'patch_embed' in k or 'enc_blocks' in k or 'enc_norm' in k:
#             if 'patch_embed.proj.weight' in k:
#                 O, I, H, W = model.backbone.patch_embed.proj.weight.shape
#                 if len(v.shape) < 4:
#                     # For old models that I trained prior to conv based patchification
