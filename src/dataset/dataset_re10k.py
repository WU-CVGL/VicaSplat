...
@dataclass
class DatasetMVImgNetCfgWrapper:
    mvimgnet: DatasetRE10kCfg
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for example in chunk:
                ...
                context_image_invalid = context_images.shape[2] < self.cfg.input_image_shape[0] or context_images.shape[3] < self.cfg.input_image_shape[1]
                target_image_invalid = target_images.shape[2] < self.cfg.input_image_shape[0] or target_images.shape[3] < self.cfg.input_image_shape[1]
                ...
                # if (
                #     torch.any(torch.isnan(extrinsics)) 
                #     or torch.any(torch.isnan(context_images))
                #     or torch.any(torch.isnan(target_images))
                # ):
                #     print(
                #         f"Skipped {scene} because of existence of NaN value."
                #     )
                #     continue
