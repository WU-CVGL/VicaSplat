...
@dataclass
class DataLoaderCfg:
    train: DataLoaderStageCfg | Dict[str, DataLoaderStageCfg]
    test: DataLoaderStageCfg
    val: DataLoaderStageCfg | Dict[str, DataLoaderStageCfg]
...
class DataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        for dataset in datasets:
            ...
            if isinstance(self.data_loader_cfg.train, dict):
                dataset_name = dataset.cfg.name
                data_loader_cfg = self.data_loader_cfg.train[dataset_name]
            else:
                data_loader_cfg = self.data_loader_cfg.train
            data_loader = DataLoader(
                dataset, 
                data_loader_cfg.batch_size,
                shuffle=not isinstance(dataset, IterableDataset),
                num_workers=data_loader_cfg.num_workers,
                generator=self.get_generator(data_loader_cfg),
                worker_init_fn=worker_init_fn,
                persistent_workers=self.get_persistent(data_loader_cfg)
            )
    ...
    def val_dataloader(self):
        ...
        for dataset in datasets:
            ...
            if isinstance(self.data_loader_cfg.val, dict):
                dataset_name = dataset.cfg.name
                data_loader_cfg = self.data_loader_cfg.val[dataset_name]
            else:
                data_loader_cfg = self.data_loader_cfg.val
