...
from src.misc.LocalLogger import LocalLogger, LocalTensorboardLogger
...
class GradientNanCheckCallback(pl.Callback):
    def on_before_optimizer_step(self, trainer, pl_module, optimizer, optimizer_idx=None):
        for name, param in pl_module.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"Detect NaN grad in param {name}, dropping grads of it to 0!")
...
def train(cfg_dict: DictConfig):
    ...
    callbacks = [GradientNanCheckCallback()]
    if cfg_dict.wandb.mode != "disabled":
        ...
        logger = LocalTensorboardLogger(save_dir=output_dir)
    ...
    seed_everything(cfg_dict.seed + trainer.global_rank)
